...
class TemplatesConfig {
	...
	public TemplatesConfig(Generator generator, String paramfile)
			throws Exception {

		this.generator = generator;

		templatesFile = TemplateEngineFactory.findKnownTemplate(
				generator.getGeneratorFolder(), paramfile);
		if (templatesFile != null) {
			templateEngine = TemplateEngineFactory.forFile(templatesFile);

			for (Entry<Object, Object> entry : FileUtils
					.inputStreamAsProperties(templatesFile.getContents())
					.entrySet()) {
				...
				if (templateName.endsWith(".generator")) {
					...
					templates
							.add(new Template(this, templateName, destination));
				}
